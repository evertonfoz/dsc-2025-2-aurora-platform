...
export class EventsService {
  ...
  async create(createEventDto: CreateEventDto, ownerUserId: number): Promise<Event> {
    ...
    if (createEventDto.registrationOpensAt && createEventDto.registrationClosesAt) {
    ...
    const event = this.eventRepository.create({
      ...
      startsAt,
      endsAt,
      registrationOpensAt: createEventDto.registrationOpensAt ? new Date(createEventDto.registrationOpensAt) : undefined,
      registrationClosesAt: createEventDto.registrationClosesAt ? new Date(createEventDto.registrationClosesAt) : undefined,
      slug,
  ...
  async findAll(query: {
    ...
    // Filters
    if (query.q) {
      qb.andWhere('(event.title ILIKE :q OR event.summary ILIKE :q)', { q: `%${query.q}%` });
    }
    if (query.from) {
    ...
    if (query.visibility) {
      qb.andWhere('event.visibility = :visibility', { visibility: query.visibility });
    } else {
      qb.andWhere('event.visibility = :visibility', { visibility: EventVisibility.PUBLIC });
    }
    if (query.state) {
  ...
  async findOneByIdOrSlug(idOrSlug: number | string, requester?: { id: number; isAdmin: boolean }): Promise<Event> {
    const qb = this.eventRepository.createQueryBuilder('event');
    ...
    if (event.state !== EventState.PUBLISHED) {
      if (!requester || (requester.id !== event.ownerUserId && !requester.isAdmin)) {
  ...
  async update(id: number, updateEventDto: UpdateEventDto, requester: { id: number; isAdmin: boolean }): Promise<Event> {
    ...
    Object.assign(event, {
      ...
      startsAt,
      endsAt,
      registrationOpensAt: updateEventDto.registrationOpensAt ? new Date(updateEventDto.registrationOpensAt) : event.registrationOpensAt,
      registrationClosesAt: updateEventDto.registrationClosesAt ? new Date(updateEventDto.registrationClosesAt) : event.registrationClosesAt,
      slug,
    });
  ...
  async publish(id: number, requester: { id: number; isAdmin: boolean }): Promise<Event> {
    const event = await this.eventRepository.findOneBy({ id });
    if (!event) {
      throw new NotFoundException('Event not found');
    }
    ...
    if (event.state !== EventState.DRAFT && event.state !== EventState.ARCHIVED) {
      throw new BadRequestException('Can only publish draft or archived events');
    }
