...
describe('UsersService – create', () => {
    ...
    it('cria com sucesso: normaliza email, compacta nome, role padrão, usa pepper e remove passwordHash no retorno', async () => {
        ...
        (repo.create as jest.Mock).mockImplementation((data) => ({ ...data, id: 1 }));
        ...
        expect(repo.findOne).toHaveBeenCalledWith({ where: { email: 'ana.silva@example.com' } });
    ...
    it('respeita role explícita', async () => {
        ...
        jest.spyOn<any, any>(service as any, 'hash').mockResolvedValue('hashed-123');
    ...
    it('hash() usa genSalt(10) e chama bcrypt.hash com a senha', async () => {
        ...
        expect((bcrypt.genSalt as jest.Mock).mock.invocationCallOrder[0])
