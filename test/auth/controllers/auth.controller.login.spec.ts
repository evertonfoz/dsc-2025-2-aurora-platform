...
describe('AuthController – login', () => {
  ...
  it('POST /auth/login → delega ao service.login', async () => {
    ...
    const body = { email: 'ok@example.com', password: 'secret' } as import('../../../src/auth/dtos/login.dto').LoginDto;
    const req = { get: (k: string) => (k === 'user-agent' ? 'ua' : undefined) } as unknown as import('express').Request;
